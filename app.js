...
const DATA_UPLOAD_LINK = "https://send2me.cn/bcgogSXC/RGSnTuBRXfiPXw";
const state = {
...
function flattenForCsv(text){
  return normalizeRecallText(text).replace(/\n+/g, ' | ').trim();
}
...
function formatWCode(n){
  return 'W' + String(n).padStart(3, '0');
}
function randomLocalCode(){
  return formatWCode(Math.floor(Math.random() * 128) + 1);
}
async function fetchAssignedCode(){
  if (!C.codeServiceUrl || /^https:\/\/你的域名/.test(C.codeServiceUrl)) {
    return { code: randomLocalCode(), mode: 'local-random', uniqueGuaranteed: false };
  }
  const resp = await fetch(C.codeServiceUrl, { method: 'GET' });
  if (!resp.ok) throw new Error('assign_code_failed');
  const data = await resp.json();
  if (!data || !data.ok || !data.code) throw new Error('assign_code_invalid');
  return { code: String(data.code).toUpperCase(), mode: 'server-assigned', uniqueGuaranteed: true };
}
...
function showParticipantForm(){
  ...
  <div class="screen"><div class="form-card">
    ...
    <div class="grid">
      <div class="field">
        <label>被试编号</label>
        <input id="subjectID" value="正在分配中..." readonly>
        <div class="inline-note">系统将自动分配 W001 - W128 之间的编号。</div>
      </div>
    ...
    <div class="status" id="codeStatus">
      正在准备被试编号。<br>
      如已配置服务器接口，系统会在 W001 - W128 中自动分配一个未占用编号。<br>
      如未配置服务器接口，则仅进行本地随机编号，不能保证绝对不重复。
    </div>
    <div class="btn-row"><button id="go" disabled>进入实验</button></div>
  </div></div>`);

  const codeInput = document.getElementById('subjectID');
  const statusBox = document.getElementById('codeStatus');
  const goBtn = document.getElementById('go');

  (async ()=>{
    try{
      const assigned = await fetchAssignedCode();
      codeInput.value = assigned.code;
      state.participant.assignedCodeMode = assigned.mode;
      state.participant.uniqueCodeGuaranteed = assigned.uniqueGuaranteed ? '是' : '否';
      if (assigned.uniqueGuaranteed) {
        statusBox.innerHTML = '已成功分配唯一被试编号：<strong>' + assigned.code + '</strong>。<br>当前为服务器自动分配模式，可避免多人都使用 W001。';
      } else {
        statusBox.innerHTML = '已生成本地随机编号：<strong>' + assigned.code + '</strong>。<br>当前未连接服务器分号接口，因此只能本地随机，不能保证绝对不重复。';
      }
      goBtn.disabled = false;
    }catch(e){
      codeInput.value = randomLocalCode();
      state.participant.assignedCodeMode = 'local-random-fallback';
      state.participant.uniqueCodeGuaranteed = '否';
      statusBox.innerHTML = '服务器编号接口不可用，已回退为本地随机编号：<strong>' + codeInput.value + '</strong>。<br>这可以避免大家都写 W001，但不能保证绝对不重复。';
      goBtn.disabled = false;
    }
  })();
  ...
  document.getElementById('go').onclick = ()=>{
    ...
    if(!/^W\d{3}$/.test(rawID)){ alert('系统分配的编号格式异常，请刷新页面重试。'); return; }
    ...
    const form = {
      ...
      note: val('note').trim(),
      groupMode: 'random',
      assignedCodeMode: state.participant.assignedCodeMode || '',
      uniqueCodeGuaranteed: state.participant.uniqueCodeGuaranteed || '否'
    };
...
function scoreRecall(){
  const target = state.selectedWords.map(w=>String(w.word || '').trim()).filter(Boolean);
  const tokens = Array.isArray(state.recall.tokens) && state.recall.tokens.length
    ? state.recall.tokens
    : tokenizeRecallText(state.recall.rawText || '');
  ...
  tokens.forEach(token=>{
    if(target.includes(token)){
      if(!seen.has(token)){
        correctWords.push(token);
        seen.add(token);
      } else {
        duplicates.push(token);
      }
    } else {
      intrusions.push(token);
    }
  });

  const missedWords = target.filter(w=>!seen.has(w));

  Object.assign(state.recall, {
    tokenizedText: tokens.join('、'),
    correctedRawTextForCsv: flattenForCsv(state.recall.rawText || ''),
    correctWords,
    missedWords,
    intrusions,
    duplicates,
    correctCount: correctWords.length,
    totalPresented: target.length,
    correctRate: target.length ? correctWords.length / target.length : 0
  });
...
async function finishExperiment(){
  ...
  bundle.json.uploadStatus = '未启用自动上传';
  const files = [
  ...
  mount(screen(`
    ...
    <div class="hint">下方会自动下载结果文件；请将下载得到的 4 个文件全部上传。若浏览器阻止下载，可点击链接手动保存。</div>
    <div class="download-list">${files.map(f=>`<a download="${f.filename}" href="${f.url}">${f.filename}</a>`).join('')}<a href="${DATA_UPLOAD_LINK}" target="_blank" rel="noopener noreferrer">上传实验数据到坚果云</a></div>
    <div class="contact-box">
      <img class="contact-qr" src="assets/contact/qq_qr.jpg" alt="主试QQ二维码">
      <div class="contact-text">
        <strong>请联系主试领取被试费用。</strong><br>
        实验结束后，请按以下步骤操作：<br>
        1. 保存本页自动下载的实验数据文件；<br>
        2. 截图保存“实验结束”页面；<br>
        3. 点击下方坚果云上传链接，上传你的实验数据文件；<br>
        4. 再使用右侧二维码添加主试 QQ，发送实验完成截图，并备注你的被试编号。<br><br>
        <a href="${DATA_UPLOAD_LINK}" target="_blank" rel="noopener noreferrer" style="color:#dbeafe;text-decoration:underline;font-weight:700;">点击这里上传实验数据（坚果云）</a>
      </div>
    </div>
    <div class="finish-note">请先上传实验数据，再联系主试领取被试费用。完成上述步骤后再关闭浏览器。</div>
  `));
  files.forEach((f, idx)=>setTimeout(()=>{ const a=document.createElement('a'); a.href=f.url; a.download=f.filename; a.click(); }, 300*(idx+1)));
}
function buildOutputs(){
  const summaryRows = [
    ['项目','结果'], ['被试编号', state.participant.subjectID], ['姓名', state.participant.participantName || ''], ['年龄', state.participant.participantAge || ''], ['性别', state.participant.participantGender || ''], ['年级', state.participant.participantGrade || ''], ['组别', state.group], ['备注', state.participant.note || ''], ['样本来源', state.participant.sampleSource || '线上'], ['编号分配方式', state.participant.assignedCodeMode || ''], ['编号唯一性保证', state.participant.uniqueCodeGuaranteed || '否'],
    ['呈现给被试的词语', toChineseList(state.selectedWords.map(w=>w.word))], ['被试自由回忆原始文本', state.recall.correctedRawTextForCsv || flattenForCsv(state.recall.rawText || '')], ['被试自由回忆分行', toChineseList(state.recall.lines || [])], ['被试自由回忆分词结果', toChineseList(state.recall.tokens || tokenizeRecallText(state.recall.rawText || ''))],
  ...
  state.selectedWords.map(w=>w.word).forEach(word=>{ const idx=(state.recall.correctWords||[]).indexOf(word); recallRows.push([state.participant.subjectID, state.group, word, idx>=0?1:0, idx>=0?idx+1:'', state.recall.correctedRawTextForCsv || flattenForCsv(state.recall.rawText || '')]); });
  const csv = rows => '\ufeff' + rows.map(r=>r.map(csvEscape).join(',')).join('\n');
  return {
    json: {
      participant: state.participant, groupLabel: state.group, sampleSource: state.participant.sampleSource || '线上', assignedCodeMode: state.participant.assignedCodeMode || '', uniqueCodeGuaranteed: state.participant.uniqueCodeGuaranteed || '否', wordListPresented: state.selectedWords, imageLog: state.imageLog,
